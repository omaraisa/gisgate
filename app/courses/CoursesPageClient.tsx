...
export default function CoursesPageClient({ initialData }: CoursesPageClientProps) {
  ...
  const fetchCourses = async (page: number) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/courses?status=PUBLISHED&limit=20&page=${page}`);
      if (!response.ok) {
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {data.courses.map((course, index) => (
                <MotionCard key={course.id || index} delay={index * 0.1}>
            ...
            {loading && (
              <div className="flex justify-center py-8">
                <motion.div
                  ...
                  className="w-8 h-8 border-4 border-secondary-400 border-t-transparent rounded-full"
                />
              </div>
            )}
